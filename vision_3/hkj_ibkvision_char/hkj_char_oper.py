        ...
        if base_box is not None:
            ...
            for i in range(0,len(one_row_boxs)):
                ...
                if is_num is True and abs(n_y-b_y)<char_height_mean:
                    ...
                    steel_no_boxes.append(one_row_boxs[i][:4])
                    if last_index != -1:
            ...
            return steel_no,steel_no_score,steel_type,steel_size,steel_no_boxes
    return '',0,'','',None
...
def get_steel_info_mini(boxes, scores, labels, classes):
    ...
    if char_width_n>0 and char_height_n>0:
        ...
        for i in range(0,len(char_boxs)):
            ...
            if center_x>base_x:
                if abs(center_y-base_y)<char_height_mean:
                    ...
                    steel_no_boxes.append(char_boxs[i][:4])
                    base_y=center_y
            #判断字符间隔
            last_char_right=char_boxs[max(0,i-1)][2]
            curr_char_left=char_boxs[i][0]
            char_interval=max(0,curr_char_left-last_char_right)
            if char_interval>char_width_mean/2:
                s_conf=False
        ...
        steel_no_score=steel_no_score/14
        if s_conf is True:
            return steel_no,steel_no_score,steel_no_boxes
    return '',0,None
