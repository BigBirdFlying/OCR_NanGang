...
def init_keras_custom_objects():
    import keras
    import hkj_ibkvision_char.char_unit.efficientnet as model
...
def preprocess_image(image, image_size, mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225]):
    # image, RGB
...
def preprocess_image_char(image, image_size, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    ...
    mean=list(np.squeeze(mean))
    std=list(np.squeeze(std))
    image -= mean
    image /= std
    
    image_o=np.zeros((image_width,image_width,3),np.float32)
    image_o[int((image_width-image_height)/2):int((image_width-image_height)/2)+image_height,:,:]=image
    top_offset=int((image_width-image_height)/2)

    image_o_height, image_o_width = image_o.shape[:2]
    if image_o_height > image_o_width:
        scale = image_size / image_o_height
        resized_height = image_size
        resized_width = int(image_o_width * scale)
    else:
        scale = image_size / image_o_width
        resized_height = int(image_o_height * scale)
        resized_width = image_size
        
    image = cv2.resize(image_o, (resized_width, resized_height))

    pad_h = image_size - resized_height
    pad_w = image_size - resized_width
    image = np.pad(image, [(0, pad_h), (0, pad_w), (0, 0)], mode='constant')
    return image, scale, left_offset,top_offset
...
def postprocess_boxes_char(boxes, scale, height, width, left_offset,top_offset):
    boxes /= scale
