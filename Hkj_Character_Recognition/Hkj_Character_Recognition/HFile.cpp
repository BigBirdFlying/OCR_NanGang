...
{	
	if(!SUCCEEDED(m_hr))
	{
		printf("无法创建DOMDocument对象，请检查是否安装了MSXML库！");
		return 0;
	}
	else
	{
...
{
	m_strLogFilePath.Format(L"%s\\Log_Char",strAppPath);
...
void HFile_log::AddLogInfo(CString strLogInfo)
{
	...
	CString strDate=L"";
	CString strTime=L"";
	strDate.Format(L"%04d-%02d-%02d",ptminfo->tm_year+1900,ptminfo->tm_mon+1,ptminfo->tm_mday);
	strTime.Format(L"%02d:%02d:%02d>>",ptminfo->tm_hour,ptminfo->tm_min,ptminfo->tm_sec);
